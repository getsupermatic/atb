...
# Part B — code-review follow-ups: share image, headers, machine-readable sync

**Status:** Delivered — see B.3. One open decision: HSTS `preload`.
**Trigger:** full-codebase review, 2026-07-31. `tsc --noEmit` and `eslint .` both
pass clean; the review found no critical defects. Six actionable items came out of
it, and three were selected: the OG image, the security headers, and the two
out-of-sync machine-readable files.

**Not in this pass** (selected against, recorded so they don't get lost):

- **The dead `/contact` and `/products/*` links.** Seven links to `/contact`
  (`Hero`, `ClosingBand`, `WaysToWork`, `Nav` ×2, `Footer`, `footerNav.More`) and
  six references to `/products/*` (`products[].href`, `footerNav.Products`) point
  at routes that do not exist. Unlike `primaryNav`, which documents its homepage
  anchors as deliberate stand-ins, these have no fallback. **This is the highest-
  impact open item on the site** — it is every primary CTA.
- **`SignupForm` is dead code**, never imported, and its own comment claims it is
  in the footer. Its honeypot field is also rendered but never read on submit.

## B.1 The three items

### 1 — A share image (review finding 5)

`app/layout.tsx` declares `openGraph` and `twitter: { card:
"summary_large_image" }` with **no `images`**, and no route adds one. Every shared
link renders with no preview, and `summary_large_image` with no image is worse
than `summary` — some clients reserve the space and leave it blank.

**Approach: one static PNG, referenced explicitly.** Not `ImageResponse` /
`app/opengraph-image.tsx`: that route needs a TTF vendored into the repo (Satori
cannot read the `.woff2` files `next/font` produces) and forces the brand hexes to
be retyped in JSX, against the brief §0 no-hardcoded-hexes rule. A static asset is
one file plus four lines of metadata.

The card is generated by screenshotting a hand-authored HTML card in headless
Chrome at 1200×630 — the same fonts and the same token values the site uses, so it
is the site's own type rendering the card rather than an approximation. The
generator is kept at `scripts/og-card.mjs` so the card can be regenerated rather
than reverse-engineered — and it reads the wordmark out of `Logo.tsx` rather than
duplicating it, so the mark cannot drift.

Composition, on the Ink field: the `at the beyond.` lockup in Cream (the real
paths from `Logo.tsx`, not a font imitation), the homepage H1 *"The future, put to
work."* in Newsreader 300, a Copper rule, and `atbeyond.com` in Instrument Sans.

Note on inheritance: `/careers` and the legal routes set only `title`,
`description` and `alternates`, so they inherit the root `openGraph` object whole
and will share this card *and its homepage title*. Acceptable for one card; worth
revisiting if per-page cards are ever wanted.

### 2 — Baseline security headers (review finding 4)

`next.config.ts` is an empty object and `vercel.json` sets only `framework`.
Nothing project-specific is set: no CSP, no `Referrer-Policy`, no
`X-Content-Type-Options`, no `Permissions-Policy`, no frame protection.

Added as a `headers()` export in `next.config.ts` (one place, rather than split
across `next.config.ts` and `vercel.json`).

**On the CSP.** A nonce-based `script-src` would need `middleware.ts` and would
force dynamic rendering on a site that is entirely static — the wrong trade. So
the CSP carries `'unsafe-inline'` in `script-src`, which is required by the two
inline JSON-LD blocks in `layout.tsx`, the per-role JSON-LD on `/careers`, and
Next's own bootstrap. **This is stated plainly rather than dressed up: the CSP
here is not an XSS defence.** What it does buy is real and worth having —
`object-src 'none'`, `base-uri 'self'`, `form-action 'self'` and
`frame-ancestors 'none'` each close a distinct class of attack that does not
depend on inline script being blocked. `'unsafe-eval'` is added in development
only; the production bundle does not need it.

**On HSTS.** `Strict-Transport-Security` is a real commitment — it locks the
domain to HTTPS for the `max-age` window. Included at two years with
`includeSubDomains`, and **deliberately without `preload`**, which is
hard to reverse and should be a explicit decision rather than a side effect of a
headers pass. Flagging rather than deciding.

### 3 — Sync the two machine-readable files (review findings 6 and 7)

**`public/llms.txt`** lists four "Key pages" that do not exist —
`/who-we-are` (no route in any form), `/what-we-do` and `/how-we-work` (homepage
anchors, not pages), and `/contact` (the broken link above). It also lists
`/insights`, which `app/sitemap.ts` deliberately excludes as a stub. A file whose
entire purpose is to be read as authoritative should not ship four dead links.
Rewritten to the real destinations, with the two homepage sections listed as the
anchors they actually are.

**`app/careers/page.tsx` JobPosting schema** sets a concrete London `jobLocation`
*and* `jobLocationType: "TELECOMMUTE"` plus `applicantLocationRequirements` on a
role the copy calls "Hybrid" (`lib/careers.ts:130`). `TELECOMMUTE` is for fully
remote roles; combining it with a physical location is a documented source of
Search Console warnings. The two remote-only fields come out, `jobLocation`
stays — which is also the more conservative reading, and matches the existing
comment's stated rule that no field should claim more than the copy does.

## B.2 To do

- [x] Generate the share card; save the generator to `scripts/og-card.mjs`
- [x] Reference it from `openGraph.images` and `twitter.images` in `app/layout.tsx`
- [x] Add `headers()` to `next.config.ts`
- [x] Rewrite the `llms.txt` key-pages block against the real routes
- [x] Drop `jobLocationType` / `applicantLocationRequirements` from the JobPosting
- [x] Verify: `tsc --noEmit`, `eslint .`, `next build`, headers present on a real
      response, and the page loads with no CSP violations in the console
- [x] Review section

## B.3 Review

Five files touched, one asset and one script added. No component, style or copy
changed — the rendered site is identical.

| File | What |
|---|---|
| `scripts/og-card.mjs` | New. Renders the share card in headless Chrome. |
| `public/images/og-card.png` | New. 1200×630, 78 KB. |
| `app/layout.tsx` | `shareImage` declared once, given to `openGraph` and `twitter`. |
| `next.config.ts` | `headers()` — CSP + five response headers. |
| `public/llms.txt` | Key pages rewritten; policies and contact split out. |
| `app/careers/page.tsx` | Two remote-only JobPosting fields removed. |

### The share card

Set on Ink: the `at the beyond.` lockup in Cream, a Copper rule, the homepage H1
in Newsreader 300, `atbeyond.com` in Instrument Sans. Cream on Ink is 16.28:1 and
Stone on Ink 12.18:1, so both clear AA comfortably; the Copper rule is decorative
and carries no text.

**The generator reads `components/brand/Logo.tsx` rather than copying it.** The
`<defs>` block, the viewBox and the twelve glyph placements are all parsed out of
that file, so the card is drawn with the real wordmark paths and re-running the
script after a logo change picks the change up. It asserts on finding exactly
twelve glyphs, so a structural change to `GLYPHS` fails loudly instead of silently
shipping a partial mark. Rendered at 2× and downsampled, which is what makes the
serif read cleanly at this size.

Wired in via a **static import** rather than a literal `/images/…` path, which is
the reasoning `Hero.tsx` already documents for its plate: the emitted URL carries a
content hash, so a regenerated card gets a new URL instead of being masked by every
social platform's cache of the old one. The import also supplies the width and
height, so those cannot drift from the file.

The headline was set at 104px on the first pass and moved to 132px — at the smaller
size the right half of the card was empty and the composition read as unbalanced.

### The headers

Verified on a real response from `next start`, on both a page and a static asset:
...
**Say the honest thing about this CSP: `'unsafe-inline'` on `script-src` means it is
not an XSS defence.** Three things need inline script — the two JSON-LD blocks in
`layout.tsx`, the per-role JSON-LD on `/careers`, and Next's bootstrap — and the fix
for that is a per-request nonce, which needs middleware, which would stop all
fourteen routes prerendering. That is a bad trade for a directive the JSON-LD would
reopen anyway. `object-src 'none'`, `base-uri 'self'`, `form-action 'self'` and
`frame-ancestors 'none'` each close a distinct attack class and do not depend on
inline script being blocked, and those are the reason this is worth shipping.

`font-src` is self-only, which holds because `next/font` self-hosts both Google
faces at build time — confirmed in the browser, where the h1 resolves to Newsreader
with nothing blocked. `'unsafe-eval'` is development-only.

`X-Frame-Options` is deliberately absent: `frame-ancestors 'none'` supersedes it in
every browser that reads the CSP, and carrying both means maintaining two spellings
of one rule.

### Verification

- `npx tsc --noEmit` — clean. `eslint .` — clean. `next build` — clean, and **all
  fourteen routes still prerender**, so the headers did not force anything dynamic.
- Emitted tags checked in the prerendered HTML: `og:image`, `og:image:width/height/alt`,
  `twitter:image` and `twitter:image:alt` all present and absolute. `/careers` and
  `/legal/privacy` inherit the card as expected.
- `TELECOMMUTE` and `applicantLocationRequirements` no longer appear anywhere in the
  built careers HTML; the `JobPosting` and its `jobLocation` still do.
- Loaded `/`, `/careers` and `/legal/privacy` in a real browser against
  `next start`: **zero console messages on all three** — no CSP violations. The
  console instrument was proved live with a probe log first, so the silence is a
  result rather than an absence of evidence. Homepage renders with the paper canvas,
  Newsreader resolved, 8 images and 3 JSON-LD blocks; the markdown privacy page
  renders its h1 and all 48 paragraphs.

### Open, and deliberately not touched

- **`Strict-Transport-Security` carries no `preload`.** Two years with subdomains is
  set, but submitting the domain to the browser-baked preload list is slow to
  undo and should be its own decision. **Needs a yes/no.**
- **The dead `/contact` and `/products/*` links stand** — still the highest-impact
  open item on the site, since it is every primary CTA. So does `SignupForm` as dead
  code with an unread honeypot.
- **Per-page share cards.** All routes inherit the homepage card *and its
  openGraph title*, because none of them declare an `openGraph` block. Fine for one
  card; a `/careers` card would need one.
- The card is a committed asset, so any change to the H1 copy or the palette needs
  `node scripts/og-card.mjs` re-run. Noted in the file's own header.
